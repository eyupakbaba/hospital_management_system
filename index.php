<!DOCTYPE html>
<html>
<head>
	<title>Hospital Management System HOME</title>
</head>
<body>
	<?php 
		include("include/header.php");
?>

	<div style="margin-top: 40px;"></div>
	<div class="container">
	<div class="col-md-12">
		<div class="row">
			<div class="col-md-3 mx-1 shadow">

				<img src="pictures/info.png" style="width: 80%; height: 80%";>

				<h5 class="text-center">We are employing for doctors</h5>

				<a href="#">
					<button class="btn btn-success my-3" style="margin-left: 30%;">Apply Now!!!</button>
				</a>
				
			</div>
			<div class="col-md-3 mx-1 shadow">
				
				<img src="pictures/patient.png" style="width: 80%; height: 80%;">

				<h5 class="text-center">We are employing for doctors</h5>

				<a href="#">
					<button class="btn btn-success my-3" style="margin-left: 30%;">Apply Now!!!</button>
				</a>
			</div>
			<div class="col-md-3 mx-1 shadow">
				
				<img src="pictures/evde-dok.jpg" style="width: 80%; height: 80%;">

				<h5 class="text-center">We are employing for doctors</h5>

				<a href="#">
					<button class="btn btn-success my-3" style="margin-left: 30%;">Apply Now!!!</button>
				</a>
			</div>
		</div>
	</div>
	</div>

</body>
</html>